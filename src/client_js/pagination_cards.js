...
function renderReservationPage (items) {
  ...
  for (const item in items) {
    const innerItems = items[item];
    for (const inner in innerItems) {
...
function createSeats (items, room) {
  const tempDiv = document.querySelectorAll('.row');
  if (tempDiv != null) {
...
function populateOccupied (items, presentationId) {
  ...
  const tempStr = reservedArr.toString();
...
async function purchase (presentationId, seats, reservationData, roomsData, room) {
  ...
  if (!validate(formDataSerialized.name)) {
    ...
    const valid = checkReservation(presentationId, reservationData, seats);
    if (valid) {
      const jsonObject = {
        presentationId: presentationId,
        seats: seats,
...
function checkReservation (presentationId, items, reservation) {
  ...
  const tempStr = reservedArr.toString();
  const tempArr = tempStr.split(',').filter(n => n);
  const reservArr = reservation.split(',').filter(n => n);
...
function refreshActiveButton () {
  ...
  const currentBtn = document.querySelector('.button.active');
  currentBtn.classList.remove('active');
  const newActiveButton = paginationElement.childNodes[currentPage + 1];
...
async function onLoad () {
  const urlLink = document.URL;
  if (urlLink.includes('reservation')) {
    const urlP = getParam(urlLink);
    const storedUrlP = localStorage.getItem('urlParam');
    if (storedUrlP != null) {
    ...
    const selectedMovieTimeValue = JSON.parse(localStorage.getItem('selectedMovieTimeValue'));
    ...
    timeSelect.addEventListener('change', async (e) => {
      ...
      const presentationIdOnChange = getPresentationId();
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const presentationIdOnSubmit = getPresentationId();
      const roomIdOnSubmit = localStorage.getItem('selectedMovieTimeValue');
      const selectedSeats = JSON.parse(localStorage.getItem('selectedSeats'));
      const seats = getSeats();
      let reservedSeatsString = '';
      seats.forEach((seat, index) => {
        for (let a = 0; a < selectedSeats.length; a++) {
          if (selectedSeats[a] == index) {
      ...
      const arr = reservedSeatsString.split(',').filter(n => n);
